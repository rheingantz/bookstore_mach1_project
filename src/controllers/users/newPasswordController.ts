...
async reserPassword(req: Request, res: Response) {
  try {
    const body = req.body
    const newPasswordBody = await resetPasswordModel(body.email, body.password);

    res.status(200).json({ resetPassword: true, email: body.email, newPassword: body.newPassword });
  } catch (error: any) {
    console.error(error);
